...
namespace F
{
    public static class FileIO
    {
        static FileIO()
        {
            InstanceT.AddAssembly(typeof(FileIO).Assembly);
        }

        public static string GetFileSize(double length)
        {
            var sizeConverters = new[] { "B", "KB", "MB", "GB" };
            var index = 0;
            ...
            return length + sizeConverters[index];
        }
        ...
        public static void WriteBase64(string path, Dictionary<int, byte[]> v)
        {
            ...
            foreach (var item in v) streamWriter.WriteLine($"{item.Key}|{Convert.ToBase64String(item.Value)}");
            streamWriter.Close();
        }
        ...
        public static ByteStream WriteBytesDict<Tkey>(string path, Dictionary<Tkey, IFSerializable> v,
            bool isWriteClassName = false) where Tkey : unmanaged
        {
            ...
            {
                s.Push(item.Key);
                if (isWriteClassName) s.Push(item.Value.GetType().FullName);
        ...
        public static ByteStream WriteBytesDict(string path, Dictionary<string, IFSerializable> v,
            bool isWriteClassName = false)
        {
            var s = new Serializable();
            s.PushLength(v.Count);
            foreach (var item in v)
        ...
        public static byte[] WriteBytesDict(Dictionary<string, IFSerializableKey> v)
        {
            ...
            })
            {
                ...
                foreach (var item in v)
                {
                    s.ByteBuff.Push(item.Key);
                    var name = item.Value.GetType().FullName;
                    s.ByteBuff.Push(name);
                    s.PushSerializable(item.Value);
                }
        ...
        /// <returns></returns>
        public static Dictionary<string, IFSerializableKey> ReadBytesDict(byte[] values)
        {
            using (var se = new SerializableKey(values))
            {
                var length = se.ReadLength();
                var dict = new Dictionary<string, IFSerializableKey>(length);
                while (length > 0)
                {
                    var key = se.Read();
                    var toClass = InstanceT.CreateInstance<IFSerializableKey>(se.Read());
                    se.ReadSerializable(ref toClass);
        ...
        /// <returns></returns>
        public static Dictionary<string, TValue> ReadBytesDict<TValue>(byte[] values, bool isReadClassName = false)
            where TValue : IFSerializable
        {
            ...
            {
                var key = se.Read();
                var toClass = isReadClassName
                    ? InstanceT.CreateInstance<IFSerializable>(se.Read())
                    : InstanceT.CreateInstance<TValue>();
        ...
        public static Dictionary<Tkey, TValue> ReadBytesDict<Tkey, TValue>(byte[] values, bool isReadClassName = false)
            where Tkey : unmanaged where TValue : IFSerializable
        {
            var se = new Serializable(values);
            var length = se.ReadLength();
        ...
        public static byte[] ReadBase64(string path, int id, string values)
        {
            ...
            {
                var bytesValue = item.Split('|');
                if (bytesValue[0] == id.ToString()) return Convert.FromBase64String(bytesValue[1]);
            }
